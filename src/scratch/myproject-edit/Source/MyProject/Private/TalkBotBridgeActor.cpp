...
public:
    ...
    bool Initialize()
    {
        ...
        UMetaHumanAudioBaseLiveLinkSubjectSettings* SubjectSettings = Source->CreateSubjectSettings<UMetaHumanAudioBaseLiveLinkSubjectSettings>();
        SubjectSettings->Mood = ParseRealtimeMood(OwnerPtr->RealtimeMood);
        SubjectSettings->MoodIntensity = FMath::Clamp(OwnerPtr->RealtimeMoodIntensity, 0.0f, 1.0f);
        SubjectSettings->Lookahead = FMath::Clamp((OwnerPtr->RealtimeLookahead / 20) * 20, 80, 240);

        SubjectKey = SourceSettings->RequestSubjectCreation(OwnerPtr->RealtimeLiveLinkSubjectName.ToString(), SubjectSettings);
